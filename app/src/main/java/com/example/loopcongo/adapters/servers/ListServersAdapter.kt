...
import com.example.loopcongo.ProfileServeurActivity
...
class ListServersAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        view.setOnClickListener {
            ...
            val intent = Intent(context, ProfileServeurActivity::class.java)
            intent.putExtra("server_id", server.id)
            intent.putExtra("serverName", server.name)
            intent.putExtra("icon", server.image)
            intent.putExtra("nbMembres", server.membres_count)
